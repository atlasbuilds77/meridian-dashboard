...
interface MeridianSignalPayload {
  signal_id: string;
  ticker: string;
  action: 'buy' | 'sell';
...
/**
 * POST /api/webhooks/meridian-signal
 *
 * Receives a signal from Meridian PM2 process and fans out execution
 * to all users who have:
 *   1. meridian_auto_execute_enabled = true
 *   2. A connected SnapTrade broker (snaptrade_user_id + snaptrade_user_secret + meridian_snaptrade_account)
 *
 * Security:
 *   - Verifies MERIDIAN_SIGNAL_WEBHOOK_SECRET (separate from Helios)
...
export async function POST(request: NextRequest) {
  ...
  const expectedSecret = process.env.MERIDIAN_SIGNAL_WEBHOOK_SECRET;
  if (!expectedSecret) {
    console.error('[MeridianWebhook] MERIDIAN_SIGNAL_WEBHOOK_SECRET not configured');
    return NextResponse.json(
  ...
  let body: MeridianSignalPayload;
  try {
  ...
  if (
    ...
    console.warn('[MeridianWebhook] Invalid secret from', clientIp);
  ...
  try {
    // --- Find all users with Meridian auto-execute enabled AND SnapTrade connected ---
    ...
    if (eligibleUsers.length === 0) {
      ...
      await logAutoExecution(client, {
        ...
        details: 'No users with meridian_auto_execute_enabled + SnapTrade connected',
      });
      return NextResponse.json({ success: true, executed: 0, failed: 0, message: 'No eligible users' });
    }

    console.log(
      `[MeridianWebhook] Executing ${body.action} ${body.ticker} for ${eligibleUsers.length} user(s)`
    );
    ...
    for (const user of eligibleUsers) {
      try {
        ...
        console.log(
          `[MeridianWebhook] ✅ Order placed for ${user.username} (${user.id}): ${orderId}`
        );
        ...
        await client.query(
          ...
          [
            ...
            userQty,
            body.price || 0,
            `Auto-execute from Meridian signal ${body.signal_id}: ${JSON.stringify({
              orderId,
              symbol: tradingSymbol,
              action: snapAction,
              reason: body.reason,
        ...
        console.error(
          `[MeridianWebhook] ❌ Execution failed for ${user.username} (${user.id}):`,
          errorMsg
        );
        ...
        errors.push(`${user.username}: ${errorMsg}`);
    ...
    console.log(
      `[MeridianWebhook] Complete: ${successCount} succeeded, ${failCount} failed out of ${eligibleUsers.length} users`
    );
    ...
    console.error('[MeridianWebhook] Fatal error:', error);
    return NextResponse.json(
  ...
  try {
    await client.query(
      `INSERT INTO auto_execution_log
         (system, signal_id, ticker, action, contract_symbol, user_id, username, status, details, created_at)
    ...
    console.error('[MeridianWebhook] Failed to log auto-execution:', err);
  }
}
