...
export async function GET(request: Request) {
  ...
  const limiterResult = await enforceRateLimit({
    request,
    name: 'helios_positions',
    limit: 60,
    windowMs: 60_000,
    userId: sessionResult.session.dbUserId,
  });
  if (!limiterResult.allowed) {
    return rateLimitExceededResponse(limiterResult, 'helios_positions');
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('[helios/positions] upstream error', res.status, text);
    ...
    console.error('[helios/positions] fetch failed:', error);
    return NextResponse.json(
