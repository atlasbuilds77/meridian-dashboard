...
export default function UserDetailPage() {
  ...
  // Calculate totals for all trades
  const totalGrossPnL = trades.reduce((sum, trade) => sum + calculateGrossPnL(trade), 0);
  const totalCommissions = trades.reduce((sum, trade) => sum + calculateCommission(trade), 0);
  const totalNetPnL = trades.reduce((sum, trade) => sum + calculateNetPnL(trade), 0);
    ...
    <div className="min-h-screen px-4 py-6 sm:px-8 sm:py-8">
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
          ...
          <Card className="border-primary/30">
            <CardHeader className="pb-2">
              <CardTitle className="text-xs uppercase tracking-wider text-muted-foreground">Gross P&L</CardTitle>
            </CardHeader>
            <CardContent>
              <div className={`text-3xl font-bold ${totalGrossPnL >= 0 ? 'text-profit' : 'text-loss'}`}>
          ...
          <Card className="border-primary/30">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-loss">-{formatCurrency(totalCommissions)}</div>
            </CardContent>
          </Card>

          <Card className="border-primary/30">
            <CardHeader className="pb-2">
              <CardTitle className="text-xs uppercase tracking-wider text-muted-foreground">Net P&L</CardTitle>
            </CardHeader>
            <CardContent>
              <div className={`text-3xl font-bold ${totalNetPnL >= 0 ? 'text-profit' : 'text-loss'}`}>
              ...
              <div className="mt-4 pt-4 border-t border-primary/20">
                <div className="flex justify-end gap-6 text-sm">
                  <div className="text-muted-foreground">
                    ...
                    <span className={`font-mono font-semibold ${totalGrossPnL >= 0 ? 'text-profit' : 'text-loss'}`}>
                      {formatCurrency(totalGrossPnL)}
                    </span>
                  </div>
                  <div className="text-muted-foreground">
                    <span className="font-medium">Total Comm:</span>{' '}
                  ...
                  <div className="text-muted-foreground">
                    ...
                    <span className={`font-mono font-bold ${totalNetPnL >= 0 ? 'text-profit' : 'text-loss'}`}>
                      {formatCurrency(totalNetPnL)}
                    </span>
                  </div>
                </div>
